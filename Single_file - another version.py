...
async def updating_datasets_w_reg_interval():
    after_parser = ''
    doc = []
    pages_api_url = "http://expertflow.com/wp-json/wp/v2/pages"
    posts_api_url = "http://expertflow.com/wp-json/wp/v2/posts" 
    pages_response = requests.get(pages_api_url)
    posts_response = requests.get(posts_api_url)
    if pages_response.ok and posts_response.ok:
        ...
        for page_data in range(len(pages_data)):
            ...
            doc.append(Document(page_content=after_parser, metadata={"source" : str(page_data)+" page_source " , "title": title , "link" : link}))
        ...
        for data_post in range(len(posts_data)):
            ...
            doc.append(Document(page_content=after_parser, metadata={"source" : str(data_post) +" post_source " , "title": title , "link" : link}))
...
while True:
    ...
    # Perform the task
    asyncio.run(updating_datasets_w_reg_interval())
